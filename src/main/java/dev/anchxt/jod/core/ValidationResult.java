...
/**
 * A monad representing the result of a validation operation. Either a Success containing the
 * validated/transformed value, or a Failure containing a list of validation errors.
 *
...
public sealed interface ValidationResult<T>
  ...
  /**
   * Transforms the success value using the given function. If this is a Failure, returns itself
   * unchanged.
   */
  <R> ValidationResult<R> map(Function<? super T, ? extends R> mapper);

  /**
   * Chains validation operations. The function is only called on success. If this is a Failure,
  ...
  record Success<T>(T value) implements ValidationResult<T> {
    ...
    public <R> ValidationResult<R> flatMap(
        Function<? super T, ? extends ValidationResult<R>> mapper) {
  ...
  record Failure<T>(List<ValidationError> errors) implements ValidationResult<T> {
    ...
    public T getValue() {
      throw new IllegalStateException(
          "Cannot get value from a failed validation. Errors: " + errors);
    }
